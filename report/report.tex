...
The need to be able to write accurate English is growing as the market is becoming more global. For native Swedish speakers it can be easy to write a word that does not make sense in a certain context, by mistake. A traditional spell checker will not detect this kind of mistakes since the words themselves often are correct, but placed in the wrong context.
...
A normal spell checker would not be able to find an error like this, but an algorithm that is looking at the probability of the order of the words might would. The standard way to implement this type of word prediction is by using the so called n-gram model. The n-gram model is due to a 
...
We anticipate that a tool that could discover this kind of mistake could be of great help. As far as we know there is no tool like this specialized in Swedes writing in English. The tool should work as a context-dependent word detection.
